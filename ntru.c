	...
	uint32_t* tmp)
{
	 ...
	 */
	/*if (bWrite)
		printf("input solve_NTRU_deepest\n");*/
...
#endif
		/*if (bWrite)
			printf("make_fg_deepest Error\n");*/
		return SOLVE_ERR_GCD;
	}
	/*if (bWrite)
		printf("make_fg_deepest OK\n");*/
	/* obsolete
	...
	if (prof->q != 1) {
			...
			|| zint_mul_small(Gp, len, prof->q) != 0)
		{
			...
			++reduce_error;
				...
				/*if (bWrite)
				{
					printf ("solve_NTRU_deepest SOLVE_ERR_REDUCE\n");


					
				}*/
//#ifdef _DEBUG
//			printf("\tSOLVE_ERR_REDUCE after gcd\n");
//#endif
		...
		/*if (bWrite)
		{
			printf("solve_NTRU_deepest SOLVE_OK\n");
	...
	unsigned depth, uint32_t* restrict tmp)
{
	...
	*/
	...
	uint32_t* t2 = (uint32_t*)((int32_t*)rt1 + n);
	fxr* rt2 = (fxr*)t2;
	if (rt2 < (rt1 + n)) {
		rt2 = rt1 + n;
	}
	...
	for (;;) {
		...
		{
			printf("after for;\n");
			printf("rt1[0] = %I64d rt1[1] = %I64d\n", rt1[0].v, rt1[1].v);
			printf("rt2[0] = %I64d rt2[1] = %I64d\n", rt2[0].v, rt2[1].v);
...
//#endif
		...
		for (size_t u = 0; u < n; u++) {
			...
			k32[u] = fxr_round(rt2[u]);
			
#if 0
			//if (k[u] > 0x7FFFFFFF || -k[u] > 0x7FFFFFFFF)
			if (k32[u] != 0)
				printf("");
#endif
		...
		if (depth == 1) {
			...
			poly_sub_scaled(logn, Ft, FGlen, ft, slen, k32, scale_k);

			poly_sub_scaled(logn, Gt, FGlen, gt, slen, k32, scale_k);
	...
	uint32_t* restrict tmp)
{
	...
	*/
	...
	for (size_t u = 0; u < n; u++) {
		...
		uint64_t x = (uint64_t) * (int32_t*)&t2[u] << (SCALE - 10);
	...
	*/
	...
	for (size_t u = 0; u < n; u++) {
		...
		uint64_t x = (uint64_t) * (int32_t*)&t1[u] << (SCALE - 10);
		//rt3[u] = fxr_of_scaled32(x);
		rt3[u].v = x;
	}
	vect_FFT(logn, rt3);
